...
Sonnet & 0.833 & 0.667 & 0.100 & --- \\
Opus & 0.833 & 0.667 & 0.222 & --- \\
...
\begin{tabular}{lcc}
\toprule
Roster & Shared memory FE & Live debate FE \\
\midrule
Original (specialist + regular) & 0.833 (n=10) & 0.033 (n=10) \\
Neutral (all identical prompts) & 0.967 (n=10) & 0.125 (n=8, in progress) \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Neutral agents are more susceptible (0.967 vs 0.833) because the specialist's skeptical prompt partially protected against contagion in the original roster. Removing that protection increases adoption. The ``swayed by majority'' language in the regular agent's prompt is not driving the effect.
...
\begin{tabular}{lccc}
\toprule
Adversary ratio & Shared memory FE & Live debate FE & Honest adoption \\
\midrule
1/6 & 0.167 (n=10) & 0.150 (n=10) & 0\% \\
2/6 & 0.650 (n=10) & 0.167 (n=2, in progress) & partial \\
3/6 (even split) & 1.000 (n=10) & 0.000 (n=3, in progress) & 100\% \\
4/6 & 1.000 (n=10) & in progress & 100\% \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

With 1 adversary, no honest agent adopts the false belief. With 2 adversaries, contagion is partial and variable (FE range 0.333--1.000 across seeds). At 3/6 (an even split), every honest agent endorses the false claim in every seed. This gradient holds with neutral prompts and no role differentiation, confirming the effect is structural.
