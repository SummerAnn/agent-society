...
\begin{table}[htbp]
\caption{What agents evaluate in each task type. The same input/output format is used for all tasks. What differs is what the claims are about and whether the model can solve the task without peer entries.}
\label{tab:task_examples}
\begin{center}
\small
\begin{tabular}{@{}lp{7.5cm}c@{}}
\toprule
\textbf{Task type} & \textbf{What agents see} & \textbf{Solve alone?} \\
\midrule
\multirow{2}{*}{\shortstack[l]{Familiar science\\(6 topics)}}
& \textbf{False claim:} ``Willpower is a depletable resource'' / ``MMR causes autism'' / etc. &  \multirow{2}{*}{\shortstack{Yes on MMR\\Partial on\\ego depletion}} \\
& \textbf{Evidence:} Published replication results, retraction notices, meta-analyses. & \\
\midrule
\multirow{2}{*}{\shortstack[l]{Computable math\\(GSM8K, 5 items)}}
& \textbf{False claim:} ``The answer is \$30.'' & \multirow{2}{*}{Yes} \\
& \textbf{Evidence:} Arithmetic word problem. Agent can compute $16{-}3{-}4{=}9$, $9{\times}2{=}18$. & \\
\midrule
\multirow{2}{*}{\shortstack[l]{Hard math\\(GSM-Hard, 5 items)}}
& \textbf{False claim:} ``The answer is [wrong number].'' & \multirow{2}{*}{\shortstack{Claude yes\\Llama no}} \\
& \textbf{Evidence:} Same structure as GSM8K but with extreme numbers (millions). & \\
\midrule
\multirow{2}{*}{\shortstack[l]{Novel calculation\\(SciTaT, 18 items)}}
& \textbf{False claim:} ``The difference is 1.09.'' (Correct: 4.5\%.) & \multirow{2}{*}{No} \\
& \textbf{Evidence:} Paper paragraph + data table. Neither alone is enough. Model has zero training knowledge about the content. & \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
