...
Same problem structures from GSM-Hard \citep{gsmhard} with numbers scaled to make computation harder: ``bakes muffins with 4,933,828 eggs.'' Claude recognizes the absurdity and rejects both answers. Llama computes mechanically and can be worn down by persistent false entries over 18 steps. Contagion rate: 0/25 for Claude Haiku (five problems, five schedules) and 6/9 for Llama 8B in a separate blind cohort (three problems, three schedules). We report the unequal cohort sizes explicitly rather than extrapolating the smaller Llama cohort to 25 cells.

\subsection{Scientific table calculations (SciTaT, 2 items)}

Numeric questions from SciTaT \citep{scitat2025} require both a paragraph and a data table to solve. Example: ``Find the difference in average accuracy between the proposed approach and the second-best method.'' The paragraph identifies which methods are which; the table has the numbers. The original two-item cohort is a pilot; in its shared-memory experiment, contagion occurs in 10/10 cells and the specialist role also endorses in all 10. The paper's screening-based robustness claim rests instead on the separately logged 30-item expansion described in Appendix~\ref{app:scitat_screen}. This distinction prevents the pilot selection process from being mistaken for an auditable benchmark-wide screen.
...
\item \textbf{Write threshold.} An agent only writes to the shared memory if its confidence exceeds the writes-memory threshold (0.30 for target agents, 0.00 for liars). This means a specialist at uncertain(0.20) never writes, while liars always write. The specialist's correct assessment is invisible in the memory.
...
\item 5/6 liars: 100\% contagion (only one target agent)
...
\midrule
What drives the outcome & Whether the memory delivers counter-evidence to the mistaken agent & Whether confident false entries overwhelm the target agents' private knowledge \\
\midrule
...
This is the central positive result in debate mode. The same 4/6 liar setup that produces contagion in 26/30 shared memory runs (FE 0.811, FE$_\text{t}$ 0.433) produces 0\% in debate. The liars break under argument pressure, and no target agent ever endorses.
...
\midrule
\multirow{4}{*}{No adoption} & Truth labels & 0/5 & Oracle label overrides peer signal \\
...
& 2-entry memory limit & 2/5 & Smaller window partially reduces adoption \\
\midrule
...
Tests whether communication topology affects ego-depletion outcomes under three format configurations (shared memory, fully connected debate, and chain debate) and two adversary ratios. Each condition--roster cohort has five schedules.

\begin{table}[htbp]
\caption{Village structure v1 results. Ego depletion, Claude Haiku, 5 schedules per condition--roster cohort and 30 cells total. FE is all-agent false endorsement; the debate and chain rows have FE$_\text{t}=0$ throughout.}
\label{tab:app_village}
\begin{center}
\small
\begin{tabular}{llccc}
\toprule
Format & Adversaries & FE & TD & Div \\
\midrule
Shared memory & 3/6 & 0.833 & 0.472 & 0.410 \\
Shared memory & 4/6 & 0.833 & 0.522 & 0.410 \\
Debate (fully connected) & 3/6 & 0.000 & 0.333 & 0.000 \\
Debate (fully connected) & 4/6 & 0.100 & 0.367 & 0.198 \\
Chain topology (debate) & 3/6 & 0.333 & 0.444 & 0.579 \\
Chain topology (debate) & 4/6 & 0.333 & 0.444 & 0.579 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

The chain cohorts have all-agent FE $=0.333$ but FE$_\text{t}=0$: the residual endorsement comes from instructed adversaries, not spread to target agents. These 30 cells therefore do not support a claim that chain topology causes target contagion; they show that all-agent FE can differ because adversary retention differs.
...
\begin{table}[htbp]
\caption{Chat collusion results across four familiar science topics (ego depletion, MMR/autism, climate attribution, and LK-99), Claude Haiku, 10 schedules per topic--condition and 40 cells per row.}
...
Condition & $n$ & FE \\
\midrule
Open discussion (standard debate) & 40 & 0.071 \\
Truthful selective collusion (hidden) & 40 & 0.046 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Selective disclosure did not increase mean false endorsement in these cohorts. The clean rerun includes only the open-discussion and hidden-collusion conditions; no separate visible-collusion result is claimed.
...
Beyond binary contagion (did the target agent endorse?), we measure four levels of influence:

\begin{enumerate}[leftmargin=*]
\item \textbf{Hard contagion:} target agent endorses the false claim at the final step.
\item \textbf{Uncertainty induction:} target agent moves from reject to uncertain. The lie did not fully take hold but the agent lost its correct conviction.
\item \textbf{Delayed rejection:} target agent rejects but took longer than the personal memory baseline to reach rejection.
\item \textbf{Confidence erosion:} target agent rejects but at lower confidence than the personal memory baseline.
\end{enumerate}
...
For the expansion, we screened 30 SciTaT items using a single Claude Haiku analyst. Each item was rendered as no-context, full-context, paragraph-only, and one or more table-only variants, giving 123 context variants. Five schedules per variant produced 615 completed screening cells. The group evaluation retained the 18 items with 0\% no-context accuracy and at least 80\% full-context accuracy. One-piece variants are included in the released screen but were not an additional inclusion gate for the expansion.

\begin{table}[htbp]
\caption{Auditable SciTaT screening and evaluation cohorts. ``Cells'' are completed scenario--condition--roster--schedule runs. Exact run IDs appear in the released manifest.}
\label{tab:scitat_screen}
\begin{center}
\small
\begin{tabular}{lrrrr}
\toprule
Stage & Items & Variants & Schedules & Cells \\
\midrule
Single-analyst screen & 30 candidates & 123 context variants & 5 & 615 \\
Group experiment & 18 selected & 3 protocols $\times$ 3 models & 5 & 810 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

The inclusion rules are operational: they show that the evaluated model was wrong without supplied evidence and usually correct with the full context under these prompts and schedules. They do not establish that the underlying papers were absent from pretraining. Because items are selected on these outcomes, subsequent contagion rates are conditional on passing the screen and must not be interpreted as an unbiased estimate over the full SciTaT benchmark.

\subsection{Expanded screening and cross-model contagion (v2)}

The screen identified 18 items that pass the two expansion gates (0\% no-context accuracy and $\geq$80\% full-context accuracy). We ran all 18 through the contagion experiment with 3 models (Claude Haiku, Mistral 8B, Llama 8B), 3 protocols, and 5 schedules (810/810 cells completed).
...
\midrule
Claude Haiku & 0.972 / 0.917 & 0.667 / 0.000 & 0.667 / 0.000 \\
Mistral 8B & 0.952 / 0.856 & 0.667 / 0.000 & 0.796 / 0.389 \\
Llama 8B & 0.913 / 0.739 & 0.667 / 0.000 & 0.852 / 0.556 \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Shared memory produces high target-agent adoption on every model (FE$_\text{t}=0.739$--$0.917$), whereas debate produces FE$_\text{t}=0$ on all 270 debate cells. Debate's all-agent FE remains 0.667 because all instructed adversaries retain their assigned false option; this is exactly why FE$_\text{t}$ is the spread metric. Personal memory reveals substantial model differences: Haiku target agents never endorse, while Mistral and Llama target agents endorse in 38.9\% and 55.6\% of final states even without peer communication. The matched shared-minus-personal adoption gaps are therefore 0.917 for Haiku, 0.467 for Mistral, and 0.183 for Llama. These gaps, rather than all-agent FE alone, quantify adoption associated with shared communication. The study establishes robustness over the evaluated items, models, prompts, and schedules rather than universal performance on SciTaT.

\textbf{Cached-summary diagnostic.} In these 270 debate runs, the legacy chat finalizer cached \texttt{falseClaimEndorsementRate=0} in \texttt{summary.json} even though the focal-claim rows in SQLite retain the four assigned adversary endorsements. The released manifest records both values and treats the scenario-declared focal claim's final per-agent trace state as canonical, yielding FE $=0.667$, FE$_\text{t}=0$, and AR $=1.000$. The target-adoption conclusion is unchanged; the stale cached all-agent FE is not used in the table.
...
\caption{Cross-model mitigations (ego depletion, 4/6 adversaries, 5 schedules each). The figure reports target-agent FE$_\text{t}$. Verification yields zero target adoption for all five models. Debate and chain topology leave nonzero target adoption only for Mistral in this grid.}
...
A key concern is whether the contagion result depends on the prompted susceptibility of the regular agent (``you rely heavily on what other researchers say''). We test this by replacing all target agents with neutral agents. The exact prompts are:
...
\caption{Neutral-prompt replication. The figure reports target-agent FE$_\text{t}$. Neutral target agents show greater adoption than the original role-differentiated roster on ego depletion and the same zero adoption on MMR. Personal-memory FE$_\text{t}$ is zero for both rosters.}
...
We vary the number of adversaries from 1 to 4 out of 6 agents, with all target agents using neutral prompts.
...
All 120 cells complete with 10 schedules per condition. With 1 adversary, no target agent adopts the false belief (FE$_\text{t} = 0.000$). With 2 adversaries, target-agent adoption increases to 47.5\%. At 3/6 (an even split) and above, every target agent endorses (FE$_\text{t} = 1.000$). Debate and personal memory show FE$_\text{t} = 0.000$ at every ratio. This gradient holds with neutral prompts and no role differentiation, showing susceptibility wording is not necessary and supporting a communication-substrate explanation.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{fig/fig_adversary_ratio.png}
\caption{Adversary-ratio gradient with neutral prompts (ego depletion, 10 schedules per cell). The figure reports target-agent FE$_\text{t}$. Shared-memory adoption rises from 0 at 1/6 adversaries to 1.000 at 3/6 and 4/6. Debate and personal-memory FE$_\text{t}$ remain zero at every ratio.}
...
\begin{table}[h]
\caption{Target-agent false endorsement rate (FE$_\text{t}$) on ego depletion by model and provenance condition (5 schedules each).}
...
The repository contains development pilots, superseded reruns, and experiment IDs reused across separately launched model cohorts. Filesystem totals and a single \texttt{*-grid-results.json} are therefore not reliable definitions of the paper sample. The generated \texttt{iclr2027/run\_manifest.json} is the canonical paper-facing source of truth, and \texttt{iclr2027/RUN\_MANIFEST.md} is its readable index. Each table cohort contains exact run IDs, experiment/configuration filters, scenario, condition, roster, model, order schedule, summary and trace paths, and SHA-256 checksums. Every included cell has one SQLite trace containing agent states, memory entries, retrievals, model calls, and chat messages where applicable. Final FE, FE$_\text{t}$, and AR are recomputed from the final per-agent state for the scenario's declared focal claim; the generator fails when a stated table value, run count, file identity, or database run identity does not match. This prevents overwritten grid summaries, repeated schedule labels, or duplicate persisted state rows from silently changing a reported denominator.
