...
Shared memory contagion is identical (FE $= 0.833$, FE$_\text{t} = 0.500$) across all five models. Verification labels eliminate non-adversarial adoption universally (FE$_\text{t} = 0.000$ on every model). The all-agent FE in debate can be misleading. GPT-4o-mini debate shows FE $= 0.667$, which appears to indicate no protection. However, FE$_\text{t} = 0.000$ for GPT-4o-mini in debate, meaning no non-adversarial agent adopted the false claim. The high all-agent FE reflects adversaries refusing to recant, not non-adversarial agents falling. Similarly, Mistral debate FE $= 0.200$ corresponds to FE$_\text{t} = 0.300$ because most adversaries abandon their position. Debate protects non-adversarial agents on all five models tested (FE$_\text{t} \leq 0.300$). Verification is the only defense that simultaneously eliminates non-adversarial adoption and leaves adversary behavior unchanged.
...
\textbf{Neutral prompt (replacement for both):} ``You are an independent analyst. Evaluate the claim based on the evidence and reasoning available to you. Form your own judgment.'' No numeric weight parameters appear in the prompt. The write-confidence threshold is set to 0.
...
A key question is whether information cascades can be prevented by tracking where each memory entry came from. We test \textbf{origin tracking} (provenance labeling), which annotates each entry with the identity of the agent whose earlier entry it was adopted from. This makes the derivation chain visible.
