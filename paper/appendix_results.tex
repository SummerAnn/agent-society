...
Llama 8B & 0.931 (n=60) & 0.000 (n=60) & 0.872 (n=56) \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Shared memory produces near-total contagion on every model (0.93--0.97). Live debate produces zero contagion on every model. This differs from familiar topics, where debate protection is model-dependent (0\% for Haiku, 23--50\% for others). On novel tasks, no agent can argue convincingly for the wrong answer, so debate defaults to uncertainty rather than false certainty. Personal memory reveals model differences in baseline capability: Haiku stays correctly uncertain (0.667, only liars endorse), while Llama guesses wrong even alone (0.872). The original 2-item finding generalizes to all 18 screened items.
