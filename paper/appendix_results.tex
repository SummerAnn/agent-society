...
\begin{table}[htbp]
\caption{Core format comparison and baselines. Ego depletion, 4/6 liars (except baselines), Claude Haiku, 30 runs for shared memory (seeds 1--20 pooled), 30 for debate, 20 for personal memory. FE$_\text{t}$ excludes adversaries from the denominator.}
\label{tab:app_core}
\begin{center}
\small
...
4/6 liars (67\%) & 30 & 0.811 & 26/30 (87\%) & 6 \\
...
Claude Haiku 4.5 & 0.833 & 0.000 & Liars break by round 3 \\
...
Sonnet & 0.833 & 0.667 & 0.100 & 0.467 \\
Opus & 0.833 & 0.667 & 0.167 & 0.100 \\
...
Shared memory contagion is identical (0.833) across all 5 models. Verification labels eliminate honest adoption universally (0.667 = adversary-only baseline on every model). Debate effectiveness varies by model: Haiku (0.000), Sonnet (0.100), and Opus (0.167) show strong protection, while Mistral (0.200) shows moderate protection and GPT-4o-mini (0.667) shows none. Chain topology varies: Opus (0.100) is the strongest, Haiku (0.333) and Sonnet/GPT (0.467) are intermediate, and Mistral (0.667) gets no benefit. This confirms verification is the only universal structural defense, while debate and topology protection depend on model training.
...
\textbf{Neutral prompt (replacement for both):} ``You are an independent analyst. Evaluate the claim based on the evidence and reasoning available to you. Form your own judgment.'' No numeric weight parameters appear in the prompt. The write-confidence threshold is set to 0, so all agents write regardless of uncertainty.
...
Original (specialist + regular) & 0.667 (n=10) & 0.000 (n=10) & 0.667 (n=10) \\
Neutral (all identical prompts) & 0.667 (n=10) & 0.000 (n=10) & 0.667 (n=10) \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

On ego depletion, personal memory is identical (0.667) for both rosters, confirming that the baseline is the same and the difference comes from the communication format. Neutral agents are more susceptible in shared memory (0.967 vs 0.833) because the specialist's skeptical prompt partially protected against contagion in the original roster. Removing that protection increases adoption. Neutral agents also show slightly more leakage in debate (0.100 vs 0.033). The ``swayed by majority'' language in the regular agent's prompt is not driving the effect.

On MMR (a well-known fact), all conditions show FE = 0.667 in shared memory and 0.000 in debate for both rosters. Training knowledge protects regardless of prompt style. The neutral-prompt effect is specific to ambiguous topics where the model lacks strong prior knowledge.
...
\midrule
1/6 & 0.167 (n=10) & 0.150 (n=10) & 0.167 (n=10) \\
2/6 & 0.650 (n=10) & 0.150 (n=10) & 0.333 (n=10) \\
3/6 (even split) & 1.000 (n=10) & 0.067 (n=10) & 0.500 (n=10) \\
4/6 & 1.000 (n=10) & 0.100 (n=10) & 0.667 (n=10) \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

All 120 cells complete with 10 seeds per condition. With 1 adversary, no honest agent adopts the false belief in any condition (shared FE = personal FE = 0.167). With 2 adversaries, shared memory nearly doubles adoption compared to personal memory (0.650 vs 0.333). At 3/6 (an even split), shared memory produces total contagion (FE = 1.000 in all 10 seeds) while personal memory shows only the adversaries endorsing (0.500). At 4/6, shared memory again shows total contagion (1.000) while debate reduces FE to 0.100. Debate protects at every ratio, with FE between 0.067 and 0.150. This gradient holds with neutral prompts and no role differentiation, confirming the effect is structural and scales with the number of confident false entries in the shared memory.
