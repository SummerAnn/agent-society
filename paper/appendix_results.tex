...
\begin{tabular}{lccc}
\toprule
Adversary ratio & Shared memory FE & Live debate FE \\
\midrule
1/6 & 0.167 (n=10) & 0.150 (n=10) \\
2/6 & 0.650 (n=10) & 0.150 (n=10) \\
3/6 (even split) & 1.000 (n=10) & 0.067 (n=10) \\
4/6 & 1.000 (n=10) & 0.000 (n=1, in progress) \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

With 1 adversary, no honest agent adopts the false belief (FE = 0.167 = the adversary alone). With 2 adversaries, contagion is partial and variable (FE range 0.333--1.000 across seeds, mean 0.650). At 3/6 (an even split), every honest agent endorses the false claim in every seed (FE = 1.000, 10/10). Debate protects at every ratio, with FE at or below 0.150. This gradient holds with neutral prompts and no role differentiation, confirming the effect is structural and scales with the number of confident false entries in the shared memory.
