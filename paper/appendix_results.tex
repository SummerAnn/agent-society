...
\item \textbf{Corroboration inflation (CI).} Ratio of endorsing memory entries to independent evidence roots. Each memory entry is traced back through its derivation chain to the earliest ancestor with no recorded parent. Entries sharing the same root are counted once:
\[
\text{CI} = \frac{|\text{endorsing entries}|}{|\text{unique roots}|}
\]
$\text{CI} = 1$ means every endorsement is an independent source. $\text{CI} = 15$ means 15 endorsements trace back to a small number of roots. High CI indicates false corroboration, where repeated testimony is mistaken for independent agreement.
\end{enumerate}
...
\midrule
Familiar science (MMR/autism) & ``MMR causes autism'' & Wakefield retracted, 650K-child study found no link. Agent knows this is a famous fraud. & Rejects --- training knowledge too strong & Rejects & Yes \\
\midrule
...
\midrule
Novel calc.\ (SciTaT, 2) & ``The difference is 1.09'' (correct: 4.5\%) & Paper paragraph + data table. Must read both and compute $11.6\%{-}7.1\%{=}4.5\%$. Model has zero training knowledge. & Endorses --- no basis to resist (0.80) & Stays uncertain --- no one can argue & No \\
\bottomrule
\end{tabular}
\end{center}
\end{table}
...
This is the central positive result in debate mode. The same 4/6 liar setup that produces contagion in 26/30 shared memory runs (FE 0.750, honest adoption 35\%) produces 0\% in debate. The liars break under argument pressure, and no honest agent ever endorses.
...
\toprule
Format & $n$ & FE (all) & Honest adopt. & Div & Contagion \\
\midrule
Shared memory (no correction) & 30 & 0.750 & 0.348 & 0.495 & 26/30 (87\%) \\
Live debate (fully connected) & 30 & 0.000 & 0.000 & 0.000 & 0/30 (0\%) \\
Personal memory & 10 & 0.667 & 0.000 & 0.579 & 0/10 (0\%) \\
\midrule
...
\begin{table}[htbp]
\caption{11 defenses tested. Ego depletion, 4/6 liars, Claude Haiku unless noted.}
...
\midrule
No cascade on strong-signal topics (MMR) & 0/30 contagion (Claude Haiku). Exception: Llama 8B cascades on MMR even in debate (FE\,=\,1.00), consistent with weaker training signal on this topic (Appendix~G.20) & Partial \\
...
A second screening round with 5 seeds per item (615 cells) identified 18 items that pass gates 1 and 3 (0\% no-context accuracy, $\geq$80\% full-context accuracy). We ran all 18 items through the contagion experiment with 3 models (Claude Haiku, Mistral 8B, Llama 8B), 3 conditions, and 5 seeds (810 cells total, 720 completed).

\begin{table}[htbp]
\caption{SciTaT contagion across 18 novel items and 3 models. FE is mean false endorsement rate across all items and seeds per condition.}
\label{tab:scitat_v2}
\begin{center}
\small
\begin{tabular}{lccc}
\toprule
Model & Shared memory & Live debate & Personal memory \\
\midrule
Claude Haiku & 0.972 (n=90) & 0.000 (n=90) & 0.667 (n=90) \\
Mistral 8B & 0.952 (n=90) & 0.000 (n=90) & 0.796 (n=90) \\
Llama 8B & 0.913 (n=90) & 0.000 (n=90) & 0.852 (n=90) \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Shared memory produces near-total contagion on every model (0.91--0.97). Live debate produces zero contagion on every model. This differs from familiar topics, where debate protection is model-dependent (0\% for Haiku, 23--50\% for others). On novel tasks, no agent can argue convincingly for the wrong answer, so debate defaults to uncertainty rather than false certainty. Personal memory reveals model differences in baseline capability: Haiku stays correctly uncertain (0.667, only liars endorse), while Llama guesses wrong even alone (0.852). The original 2-item finding generalizes to all 18 screened items and all 3 models (270 cells each, 810 total).
...
The main paper tests mitigations primarily on Claude Haiku. We replicate the top defenses across 5 additional models.

\begin{table}[htbp]
\caption{Cross-model mitigation results (ego depletion, 4/6 adversaries, 5 seeds per cell). Verification labels reduce FE to 0.667 (adversary-only baseline) on every model tested, confirming this defense is universal.}
...
Sonnet & 0.833 & 0.667 & 0.100 & 0.333 \\
Opus & 0.833 & 0.667 & 0.167 & --- \\
Mistral 8B & 0.833 & 0.667 & 0.200 & 0.667 \\
...
Shared memory contagion is identical (0.833) across all models. Verification labels eliminate honest adoption universally. Debate effectiveness varies by model: Haiku and Sonnet show strong protection (0.000--0.100), while GPT-4o-mini shows none (0.667). This confirms the paper's claim that verification is the only universal structural defense, while debate protection depends on model training.
...
A key concern is whether the contagion result depends on the prompted susceptibility of the regular agent (``you rely heavily on what other researchers say''). We test this by replacing all honest agents with neutral agents whose prompt contains no social-weight instructions, no personality, and no role differentiation. The prompt reads: ``You are an independent analyst. Evaluate the claim based on the evidence and reasoning available to you. Form your own judgment.'' No numeric weight parameters appear in the prompt.

\begin{table}[htbp]
\caption{Neutral-prompt replication (ego depletion, 4/6 adversaries, 10 seeds). Neutral agents show equal or greater contagion than role-differentiated agents, confirming the effect is not prompted into existence.}
\label{tab:neutral_prompt}
\begin{center}
\small
\begin{tabular}{lcc}
\toprule
Roster & Shared memory FE & Live debate FE & Personal memory FE \\
\midrule
Original (specialist + regular) & 0.833 (n=10) & 0.033 (n=10) & 0.667 (n=10) \\
Neutral (all identical prompts) & 0.967 (n=10) & 0.100 (n=10) & 0.667 (n=10) \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

Personal memory is identical (0.667) for both rosters, confirming that the baseline is the same and the difference comes from the communication format. Neutral agents are more susceptible in shared memory (0.967 vs 0.833) because the specialist's skeptical prompt partially protected against contagion in the original roster. Removing that protection increases adoption. Neutral agents also show slightly more leakage in debate (0.100 vs 0.033). The ``swayed by majority'' language in the regular agent's prompt is not driving the effect.

On MMR (a well-known fact), both rosters show FE = 0.667 in shared memory (no contagion), confirming that training knowledge protects regardless of prompt style. The neutral-prompt effect is specific to ambiguous topics where the model lacks strong prior knowledge.
...
\begin{table}[htbp]
\caption{Adversary ratio gradient with neutral prompts (ego depletion, 10 seeds per cell). Contagion emerges at 2/6 and reaches 100\% at 3/6 (even split).}
\label{tab:adversary_ratio}
\begin{center}
\small
\begin{tabular}{lccc}
\toprule
Adversary ratio & Shared memory FE & Live debate FE & Personal memory FE \\
\midrule
1/6 & 0.167 (n=10) & 0.150 (n=10) & --- \\
2/6 & 0.650 (n=10) & 0.150 (n=10) & --- \\
3/6 (even split) & 1.000 (n=10) & 0.067 (n=10) & 0.500 (n=8) \\
4/6 & 1.000 (n=10) & 0.000 (n=1) & --- \\
\bottomrule
\end{tabular}
\end{center}
\end{table}

With 1 adversary, no honest agent adopts the false belief (FE = 0.167 = the adversary alone). With 2 adversaries, contagion is partial and variable (FE range 0.333--1.000 across seeds, mean 0.650). At 3/6 (an even split), every honest agent endorses the false claim in every seed (FE = 1.000, 10/10). The 3/6 personal memory baseline is 0.500 (3 adversaries endorse, 3 honest agents do not), confirming the shared memory effect is real and not an artifact of the ratio. Debate protects at every ratio, with FE at or below 0.150. This gradient holds with neutral prompts and no role differentiation, confirming the effect is structural and scales with the number of confident false entries in the shared memory.

% ============================================================
\section{Provenance Defense: Origin Tracking as a Mitigation}
\label{app:provenance}
% ============================================================

A key question is whether information cascades can be prevented by tracking where each memory entry came from. We introduce \textbf{origin tracking} (provenance labeling), which annotates each entry with the identity of the agent whose earlier entry it was adopted from. This makes the derivation chain visible, so an agent reading the shared memory can distinguish independent assessments from repeated testimony.
...
\item \textbf{Lineage collapsed}. Entries that trace back to the same original root are deduplicated. Each displayed entry represents one independent source.
\item \textbf{Provenance aware}. Each entry is annotated with its derivation origin (e.g., ``derived from analyst\_1 at step 2''). A warning explains that derived entries are not independent observations and advises counting independent roots, not agreeing agents.
\end{itemize}
...
Provenance labeling eliminates target-agent contagion on 5 of 6 topics. The exception is PANDAS, where all four conditions produce similar rates (0.300--0.400), suggesting the cascade on that topic operates through a mechanism other than false corroboration.

\textbf{Corroboration inflation.} We define corroboration inflation as the ratio of endorsing memory entries to independent evidence roots. In the naive condition, ego depletion runs show inflation of 13--15 (13--15 endorsements tracing back to 1--2 roots). Provenance labeling does not reduce this ratio directly, but makes it visible to the reading agent.

\subsection{Ablation: annotation versus instruction}

To test whether the effect comes from the derivation annotations or from the warning prompt, we compare three conditions across the same 6 topics (90 cells).
...
\begin{table}[h]
\caption{Ablation: annotation alone versus annotation with explanation (6 topics, 5 seeds, Claude Haiku).}
...
The annotation alone provides a small reduction (0.217 to 0.167). The full effect requires the explanation (0.217 to 0.067). On ego depletion specifically, annotation alone drops contagion from 0.500 to 0.300 (a 40\% reduction), but only the full warning eliminates it. This indicates that LLMs can partially interpret provenance metadata, but do not reliably discount non-independent sources without explicit instruction.
...
Provenance-aware labeling eliminates contagion on both Claude models (Haiku and Sonnet) but only reduces it on GPT-4o-mini (0.500 to 0.200). The minimal annotation has no effect on GPT-4o-mini, suggesting GPT does not spontaneously interpret derivation metadata. MMR shows zero contagion across all models and conditions, consistent with the main paper's finding that well-known false claims resist contagion regardless of format.
...
Total cells (experiment $\times$ condition $\times$ seed) & 9,414 \\
SQLite trace databases & 9,445 \\
Unique scenarios & 21 \\
...
The main text refers to 58 experiment configurations, 14 condition files, and 86 scenario definitions. These are the source files released with the code. Each experiment configuration defines a factorial grid (e.g., 3 scenarios $\times$ 4 conditions $\times$ 2 rosters $\times$ 5 seeds), which expands to the 178 total experiments and 9,414 cells above. The 48 unique conditions include variants created by the factorial expansion (e.g., the same condition file paired with different rosters). The 21 unique scenarios count distinct task families; the 86 scenario definition files include per-topic and per-context-level variants within each family.
